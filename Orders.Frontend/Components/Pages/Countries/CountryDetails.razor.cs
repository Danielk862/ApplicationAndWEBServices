...
namespace Orders.Frontend.Components.Pages.Countries
{
    ...
    {
        private Country? country; private List<State>? states;
        ...
        {
            var responseHttp = await Repository.GetAsync<Country>($"/api/countries/{CountryId}"); if (responseHttp.Error)
            {
            ...
            country = responseHttp.Response; return true;
        }
        ...
        private async Task ShowModalAsync(int id = 0, bool isEdit = false)
        {
            ...
            if (result!.Canceled!)
            {
                await LoadTotalRecordsAsync(); await table.ReloadServerData();
            }
        }
